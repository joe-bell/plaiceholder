...
- `options`: _(optional)_
  ...
  - Sharp Configuration
    ...
    - `removeAlpha`: remove alpha channel for transparent images (default: `true`)
      ...
      _Note: this option is a [no-op for Blurhash](/#tradeoffs)._

    - `saturation`: saturation multiplier (default: `1.2`)

    - `autoOrient`: automatically orient image base on EXIF data (default: `false`)

      \_Note: this option is a no-op for images without EXIF DATA. More info in [Sharp documentation](https://sharp.pixelplumbing.com/api-operation#rotate)

    - `fit`: (similar to CSS `object-fit`) option passed to [Sharp resize method](https://sharp.pixelplumbing.com/api-resize#resize).
